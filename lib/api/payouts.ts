...
// ---------- UPDATE PAYOUT ----------
export async function updatePayout(payoutId: number, storeId: number) {
  const supabase = await createServerClient();
...
export const getTodaysPayoutsCached = unstable_cache(
  async (storeOptions: OptionsType, storeName: string) => {
    ...
    const payoutData = ((data as unknown as GetTodaysPayoutsType[]) ?? []).map(
      (p) => ({
        id: p.id,
...
// ---------- GET PAYOUTS TO POST ----------
export const getPayoutsToPostCached = unstable_cache(
  async (storeOptions: OptionsType, storeName: string) => {
    const storeId = storeOptions.find((s) => s.name === storeName)?.id;
    if (!storeId) return { payoutData: [] };
    ...
    payoutData = (sortedPayouts as unknown as GetPayoutsToPostType[]).map(
      (p) => ({
        id: p.id,
        invoice_number: p.invoice_number,
        amount: p.amount,
